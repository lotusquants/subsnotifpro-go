...
func (s *appstoreSubscriptionService) handleRefundDeclined(
	ctx context.Context,
	tx *gorm.DB,
	...
	if payload.OriginalTransactionId == "" {
		return nil, fmt.Errorf("missing original transaction ID in refund declined notification")
	}
	...
	reason := "Refund request was declined by Apple"
	if err := subscription.AddEvent(tx, models.EventTypeRefundDeclined, notification, reason); err != nil {
		return nil, fmt.Errorf("failed to add refund declined event: %w", err)
	}

	logger.Log.Infof("Processed refund declined for subscription %s (user: %s, original transaction ID: %s)",
		subscription.ID, subscription.UserID, payload.OriginalTransactionId)

	// // Post-processing
	// if err := s.notificationService.SendRefundDeclinedNotification(
	//     ctx,
	//     subscription.UserID,
	// ); err != nil {
	//     logger.Log.Warnf("Failed to send refund declined notification: %v", err)
	// }

	// // Restore any access that might have been revoked
	// if err := s.accessService.RestoreAccess(
	//     ctx,
	//     subscription.UserID,
	//     subscription.ProductID,
	// ); err != nil {
	//     logger.Log.Errorf("Failed to restore access after refund declined: %v", err)
