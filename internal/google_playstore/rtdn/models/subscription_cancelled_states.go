...
type SubscriptionCancellationHistory struct {
	ID                    uuid.UUID `gorm:"type:uuid;default:gen_random_uuid();primaryKey"`
	SubscriptionID        uuid.UUID `gorm:"type:uuid;not null;index"`
	CancellationContextID uuid.UUID `gorm:"type:uuid;not null;index"` // Reference to the cancellation context at that time

	CancellationReason CancellationReason `gorm:"type:varchar(50);not null"`
	CancelTime         time.Time          `gorm:"not null"`

	CancelSurveyReason    *CancelSurveyReason `gorm:"type:varchar(50);null"`  // Optional
	CancelSurveyUserInput *string             `gorm:"type:varchar(255);null"` // Optional freeform input

	ResubscribedAt *time.Time `gorm:"null"` // When the user resubscribed, if applicable
