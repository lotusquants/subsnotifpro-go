...
import (
	...
	"subsnotifpro-go/internal/google_playstore/rtdn/parser"
...
func (h *RTDNHandler) processEvent(c *gin.Context, event rtdnModels.GooglePlayWebhookEvent, ch *amqp.Channel) {
	defer func() {
		if r := recover(); r != nil {
...
func GetDLQSize(c *gin.Context) {
	conn, ch, err := messaging.GetAdminChannel()
	if err != nil {
