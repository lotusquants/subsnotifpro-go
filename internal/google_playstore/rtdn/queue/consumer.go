...
func (c *Consumer) processMessage(ctx context.Context, msg amqp.Delivery) {
	...
	if err := json.Unmarshal(msg.Body, &event); err != nil {
		...
		if err := msg.Nack(false, false); err != nil {
			logger.Log.Errorf("⚠️ Failed to send message to DLQ: %v", err)
		}
		return
	}
	...
	if msgRetryCount >= constants.MaxRetries {
		...
		if err := c.repo.UpdateWebhookStatus(ctx, event.ID, "movedToDLQ"); err != nil {
	...
	// Process event via service
	err := c.service.ProcessWebhookEvent(event)
	if err != nil {
	...
	// Update DB Status via Repository
	if err := c.repo.UpdateWebhookStatus(ctx, event.ID, "completed"); err != nil {
		logger.Log.Errorf("⚠️ Failed to update status for event %s: %v", event.ID, err)
	}
